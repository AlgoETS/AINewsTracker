...
def test_logger_level():
    ...
    assert std_logger.getEffectiveLevel() == logging.INFO, "Logger default level is not INFO"

    logger_instance.set_level(logging.DEBUG)
    assert std_logger.getEffectiveLevel() == logging.DEBUG, "Logger level is not properly set"
...
def test_logger_level_parameterized(level):
    ...
    std_logger = logger_instance.get_std_logger()  # get the standard logger
    assert std_logger.getEffectiveLevel() == level, f"Logger level is not properly set for level: {level}"
