...
def test_make_api_request(mock_client, status_code, response_data, expected, api_details):
    ...
    if status_code != 200:
        mock_response.raise_for_status.side_effect = httpx.HTTPStatusError("error", request=mock.Mock(), response=mock_response)
    else:
